...
if($database_name != ""){
    ...
    $sql1="SELECT SUM(`opt_12`) as `opening_sale` FROM `$purchase_table` WHERE `opt_5`='$product_id'"." AND `added_for`='$business'";
    $result1 = mysqli_query($local_conn_db, $sql1);
    if($result1->num_rows > 0){
        while($row1 = $result1->fetch_assoc()){
    ...
    if(($opening_purchase - $opening_sale) >= $quantity){
        ...
        echo "Not existed in pending purchase order...!--sp--".($opening_purchase - $opening_sale);
    }
}
?>